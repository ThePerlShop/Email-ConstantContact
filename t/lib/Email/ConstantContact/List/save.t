...
t::lib::Email::ConstantContact::List::save->runtests;


BEGIN {
package t::lib::Email::ConstantContact::List::save;
use strict;
use warnings;
...
t::lib::Email::ConstantContact::List::save - Unit test the C<< Email::ConstantContact::List->save() >> method.

=head1 SYNOPSIS

    # run all tests  
    prove -lv t/lib/Email/ConstantContact/List/save.t

    # run single test method
    TEST_METHOD=test_METHOD_NAME prove -lv t/lib/Email/ConstantContact/List/save.t
        ...
        [
            {
                ...
                content => xml({
                    ...
                    content => {
                        ...
                        ContactList => superhashof({
                            xmlns => 'http://ws.constantcontact.com/ns/1.0/',
                            id => [ ( $list_data{id} ) x 2 ],
                            ( map { $_ => $list_data{$_} } qw(
                                Name ShortName
                                OptInDefault DisplayOnSignup SortOrder
                            ) ),
                        }),
                    },
                }),
